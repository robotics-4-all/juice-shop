...
export const SNIPPET_PATHS = Object.freeze([
  "./server.ts",
  "./routes",
  "./lib",
  "./data",
  ...
  for (const currPath of paths) {
    if ((await fs.lstat(currPath)).isDirectory()) {
      ...
      try {
        ...
        logger.warn(
          `File ${currPath} could not be read. it might have been moved or deleted. If coding challenges are contained in the file, they will not be available.`
        );
      }
    }
  }
...
function getCodingChallengeFromFileContent(source: string, challengeKey: string) {
  const snippets = source.match(
    `[/#]{0,2} vuln-code-snippet start.*${challengeKey}([^])*vuln-code-snippet end.*${challengeKey}`
  );
  if (snippets == null) {
