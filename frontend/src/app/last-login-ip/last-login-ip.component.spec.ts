...
describe("LastLoginIpComponent", () => {
  ...
  xit("should set Last-Login IP from JWT as trusted HTML", () => {
    // FIXME Expected state seems to leak over from previous test case occasionally
    localStorage.setItem("token", process.env.LAST_LOGIN_TEST_JWT_FIRST);
    component.ngOnInit();
  ...
  xit("should not set Last-Login IP if none is present in JWT", () => {
    // FIXME Expected state seems to leak over from previous test case occasionally
    localStorage.setItem("token", process.env.LAST_LOGIN_TEST_JWT_SECOND);
    component.ngOnInit();
