    ...
    try {
      ...
      if (req.body.paymentMode === "card") {
        ...
        if (
          ...
          (card.expYear === new Date().getFullYear() && card.expMonth - 1 < new Date().getMonth())
        ) {
      ...
      user
        .update({ role: security.roles.deluxe, deluxeToken: security.deluxeToken(user.email) })
        .then((user) => {
          ...
          res.status(200).json({
            status: "success",
            data: { confirmation: "Congratulations! You are now a deluxe member!", token: updatedToken }
          });
        })
        .catch(() => {
          res.status(400).json({ status: "error", error: "Something went wrong. Please try again!" });
