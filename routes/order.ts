      ...
      .then(async (basket: BasketModel | null) => {
        if (basket != null) {
          ...
          basket.Products?.forEach(({ BasketItem, price, deluxePrice, name, id }) => {
            if (BasketItem) { // Έλεγχος για undefined
              QuantityModel.findOne({ where: { ProductId: BasketItem.ProductId } })
                .then((product) => {
                  if (product) {
                    const newQuantity = product.quantity - BasketItem.quantity;
                    QuantityModel.update({ quantity: newQuantity }, { where: { ProductId: BasketItem.ProductId } })
          ...
          const deliveryMethod: DeliveryMethod = {
            id: req.body.orderDetails?.deliveryMethodId || 0,
            deluxePrice: 0,
            price: 0,
            eta: 5,
          };
          ...
          doc.font('Times-Roman', 15).text(
            `(${req.__('The bonus points from this order will be added 1:1 to your wallet ¤-fund for future purchases!')}`,
          );
          doc.moveDown();
          doc.moveDown();
          ...
          if (req.body.UserId) {
            if (req.body.orderDetails?.paymentId === 'wallet') {
              ...
              if (wallet && wallet.balance >= totalPrice) {
                WalletModel.decrement({ balance: totalPrice }, { where: { UserId: req.body.UserId } }).catch((error) =>
                  next(error),
                );
              } else {
            ...
            WalletModel.increment({ balance: totalPoints }, { where: { UserId: req.body.UserId } }).catch((error) =>
              next(error),
            );
          }
...
function calculateApplicableDiscount(basket: BasketModel, req: Request) {
  if (security.discountFromCoupon(basket.coupon)) {
    ...
    challengeUtils.solveIf(challenges.forgedCouponChallenge, () => discount >= 80);
    return discount;
