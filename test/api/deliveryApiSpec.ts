...
function loginAndSetAuthHeader(email: string, password: string) {
...
describe('/api/Deliverys', () => {
  describe('for regular customer', () => {
    beforeAll(() => {
      return loginAndSetAuthHeader('jim@' + config.get<string>('application.domain'), 'ncc-1701');
    });
  ...
  describe('for another customer', () => {
    beforeAll(() => {
      return loginAndSetAuthHeader('another@' + config.get<string>('application.domain'), 'another-password');
    });
    ...
    it('GET delivery method', () => {
      return frisby.get(API_URL + '/Deliverys/2', { headers: authHeader })
        ...
        .then(({ json }) => {
          ...
          expect(json.data.price).toBe(1.0)
          expect(json.data.eta).toBe(0.5)
        });
    });
  });
