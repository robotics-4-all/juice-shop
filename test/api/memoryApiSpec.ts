...
// Utility function to post a new memory with an image
const postNewMemory = (filePath: string, caption: string, token: string) => {
...
describe("/rest/memories", () => {
  it("GET memories via public API", () => {
    return frisby.get(REST_URL + "/memories").expect("status", 200);
  });
  ...
  it("POST new memory image file invalid type", () => {
    ...
    return frisby
      ...
      .expect("status", 200)
      .then(({ json: jsonLogin }) => {
        return postNewMemory(file, "Valid Image", jsonLogin.authentication.token).expect("status", 500);
      });
  });
  ...
  it("POST new memory with valid for JPG format image", () => {
    ...
    return frisby
      ...
      .expect("status", 200)
      .then(({ json: jsonLogin }) => {
        return postNewMemory(file, "Valid Image", jsonLogin.authentication.token)
          .expect("status", 200)
          .then(({ json }) => {
