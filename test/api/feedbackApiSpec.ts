...
const submitFeedback = (captcha: any, body: any, headers: any = jsonHeader) => {
  return frisby.post(API_URL + '/Feedbacks', {
    headers,
    body: { ...body, captchaId: captcha.captchaId, captcha: captcha.answer }
  })
}

describe('/api/Feedbacks', () => {
  it('GET all feedback', () => {
    return frisby.get(API_URL + '/Feedbacks').expect('status', 200)
  })

  it('POST sanitizes unsafe HTML from comment', () => {
    return fetchCaptcha().then(captcha => {
      return submitFeedback(captcha, {
        comment: 'I am a harm<script>steal-cookie</script><img src="csrf-attack"/><iframe src="evil-content"></iframe>less comment.',
        rating: 1
      })
        .expect('status', 201)
        .expect('json', 'data', { comment: 'I am a harmless comment.' })
    })
  })
  ...
  it('POST feedback in another user’s name as anonymous user', () => {
    return fetchCaptcha().then(captcha => {
      return submitFeedback(captcha, {
  ...
  it('POST feedback in a non-existing user’s name fails with constraint error', () => {
    return fetchCaptcha().then(captcha => {
      return submitFeedback(captcha, {
        comment: 'Pickle Rick says your express-jwt 0.1.3 is broken!',
        rating: 0,
        UserId: 4711
      })
        .expect('status', 500)
  ...
  it('POST feedback is associated with current user', () => {
    return frisby.post(REST_URL + '/user/login', {
      ...
      .then(({ json: login }) => {
        return fetchCaptcha().then(captcha => {
          return submitFeedback(captcha, {
            comment: 'Stupid JWT secret and being typosquatted!',
            rating: 5,
            UserId: 4
          }, { Authorization: 'Bearer ' + login.authentication.token, 'content-type': 'application/json' })
            .expect('status', 201)
  ...
  it('POST feedback can be created without comment', () => {
    return fetchCaptcha().then(captcha => {
      return submitFeedback(captcha, { rating: 1 })
        .expect('status', 201)
        .expect('json', 'data', { comment: null, rating: 1 })
    })
  })

  it('POST feedback cannot be created without rating', () => {
    return fetchCaptcha().then(captcha => {
      return submitFeedback(captcha, {})
        .expect('status', 400)
        .then(({ json }) => {
  ...
  it('POST feedback cannot be created with wrong CAPTCHA answer', () => {
    return fetchCaptcha().then(captcha => {
      return frisby.post(API_URL + '/Feedbacks', {
        headers: jsonHeader,
        body: { rating: 1, captchaId: captcha.captchaId, captcha: captcha.answer + 1 }
      }).expect('status', 401)
    })
  })

  it('POST feedback cannot be created with invalid CAPTCHA id', () => {
    return submitFeedback({ captchaId: 999999, answer: 42 }, { rating: 1 }).expect('status', 401)
  })
})
...
describe('/api/Feedbacks/:id', () => {
  ...
  it('GET existing feedback by id', () => {
    return frisby.get(API_URL + '/Feedbacks/1', { headers: authHeader }).expect('status', 200)
  })

  it('DELETE existing feedback', () => {
    return fetchCaptcha().then(captcha => {
      return submitFeedback(captcha, { comment: 'I will be gone soon!', rating: 1 })
        .expect('status', 201)
        .then(({ json }) => {
          return frisby.del(API_URL + '/Feedbacks/' + json.data.id, { headers: authHeader }).expect('status', 200)
        })
    })
  })
})
